...
deployments =\
    {
        'prod':
            {
                'host_string': "ubuntu@shop.we-got-game.com",
...
def deploy(deployment="prod", branch="master"):
    ...
    #run_tests(branch)
    code_dir = '~/srv/bamboo-stem'
    with settings(warn_only=True):
        if run("test -d %s" % code_dir).failed:
            run("git clone git@github.com:larryweya/bamboo-stem.git %s" % code_dir)
    with cd(code_dir):
        ...
        # reload server
        run_in_virtual_env("uwsgi --reload /var/run/stem_uwsgi.pid")
